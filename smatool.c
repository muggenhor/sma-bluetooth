            ...
            while ((row = mysql_fetch_row(res)))  //Need to update these
            {
		        ...
		        curl_easy_cleanup(curl);
                        if( result==0 ) 
                        {
                           ...
                           if (flag.debug == 1) printf("%s\n",SQLQUERY);
                           MYSQL_RES* res1 = DoQuery(conn, SQLQUERY);
